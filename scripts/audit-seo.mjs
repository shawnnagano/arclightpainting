...
for (const route of routes) {
  if (!route.title || route.title.length > 70) errors.push(`${route.path}: title missing or too long (${route.title?.length || 0})`);
  if (!route.description || route.description.length < 120 || route.description.length > 180) errors.push(`${route.path}: description length ${route.description?.length || 0}`);
...
console.log(`SEO audit passed for ${routes.length} routes: unique titles, unique descriptions, and description lengths in range.`);
